...
class AddrmanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.nodes[0].assert_debug_log([
                "Renamed corrupt peers.dat to peers.corrupt.dat",
                "Created new peers.dat after corruption was detected",
        ]):
            self.start_node(0)
        assert_equal(self.nodes[0].getnodeaddresses(), [])
        assert os.path.exists(os.path.join(self.nodes[0].datadir, self.chain, "peers.corrupt.dat"))
