...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    LogPrint(nOffsetSample ? "time":"time2", "added time data, samples %d, offset %+d (%+d minutes)\n", vTimeOffsets.size(), nOffsetSample, nOffsetSample/60);
	...
	std::string strTimeOffsets;
        BOOST_FOREACH(int64_t n, vSorted)
	    strTimeOffsets += strprintf("%+d  ", n);
        
        LogPrint(nTimeOffset ? "time":"time2", "%s|  nTimeOffset = %+d  (%+d minutes)\n", strTimeOffsets, nTimeOffset, nTimeOffset/60);
    }
}
