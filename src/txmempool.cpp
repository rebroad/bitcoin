                                                  ...
                                                  std::string &errString) const
{
    ...
    while (!staged_ancestors.empty()) {
        ...
        if (stageit->GetSizeWithDescendants() + entry_size > limitDescendantSize) { // REBTODO - where is limitDescendantSize defined?
...
CFeeRate CTxMemPool::GetMinFee(size_t sizelimit) const {
    ...
    if (time >= lastRollingFeeUpdate + 10) {
        double newrolling = bumpedMinimumFeeRate * ((1.0 * DynamicMemoryUsage() / sizelimit) - 0.7) * 5; // REBTODO - make it 12MB below sizelimit before reducing
        if (newrolling < rollingMinimumFeeRate) {
            ...
            if (rollingMinimumFeeRate < 0)
                rollingMinimumFeeRate = 0;
        } else if (bumpedMinimumFeeRate > rollingMinimumFeeRate && newrolling >= bumpedMinimumFeeRate)
