...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, block, cmpctblock, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tx, tor, zmq"; // Don't translate these and qt below
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (chainActive.Tip() == NULL) {
        ...
        LogPrintf("%s: chainActive.Tip() == NULL, so calling ActivateBestChain\n", __func__);
        if (!ActivateBestChain(state, chainparams)) {
            LogPrintf("Failed to connect best block");
            StartShutdown();
        }
    } else
        LogPrintf("%s: chainActive.Tip() != NULL, so skipping ActivateBestChain\n", __func__);
