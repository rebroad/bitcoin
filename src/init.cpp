...
using node::DEFAULT_STOPAFTERIBD;
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-par=<n>", strprintf("Set the number of script verification threads (%u to %d). 0 = auto (cores-1). <0 = leave that many cores free. Fraction or %% = leave that fraction of cores free (e.g. -0.5 or -50%% leaves half free). Default: %d)",
    ...
    argsman.AddArg("-blockrelaypeers=<n>", strprintf("Number of outbound block-relay-only peers to maintain (default: %u). Can be changed at runtime via reloadconfig.", MAX_BLOCK_RELAY_ONLY_CONNECTIONS), ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    argsman.AddArg("-historicalbackfillinflightmib=<n>", "Per-peer target MiB of historical backfill block data to keep in flight, converted to a block count using a rolling average block size (default: 24). Can be changed at runtime via reloadconfig.", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-bootstrap=<ip>", "Attempt bootstrap recovery from a trusted peer over normal P2P headers sync by connecting to the specified peer", ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-discover", "Discover own IP addresses. Set to 1 for all interface addresses, 0 to disable, or defaultonly to only use source addresses selected toward the default gateway (default: 1 when listening and no -externalip or -proxy)", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-numonion=<n>", "Number of onion addresses to create (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-peerstatssnapshots", strprintf("Use rotating snapshots for peer MP%%/MPpm calculations instead of lifetime averages (default: %u)", 0), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-sipadisconnects", "Allow sipa disconnect logic to prevail (default: disabled)", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-stopafteribd", strprintf("Stop running after initial block download (default: %u)", DEFAULT_STOPAFTERIBD), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-maxtipage=<n>", strprintf("Maximum tip age in seconds to consider node in initial block download (default: %d)", DEFAULT_MAX_TIP_AGE), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-ibdtimethreshold=<n>", strprintf("Time threshold in minutes to consider node in initial block download (default: %d)", DEFAULT_IBD_TIME_THRESHOLD / 60), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-printpriority", strprintf("Log transaction fee rate in " + CURRENCY_UNIT + "/kvB when mining blocks (default: %u)", DEFAULT_PRINTPRIORITY), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-allowmissingdustinputs", "Allow block validation to tolerate missing inputs presumed pruned dust (consensus-changing, experimental; default: 0)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-utxodustallowed", "Store dust outputs in the UTXO set (default: 1)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-bytespersigop", strprintf("Equivalent bytes per sigop in transactions for relay and mining (default: %u)", DEFAULT_BYTES_PER_SIGOP), ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-datacarrier", strprintf("Relay and mine data carrier transactions (default: %u)", DEFAULT_ACCEPT_DATACARRIER), ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-datacarriersize", strprintf("Maximum size of data in data carrier transactions we relay and mine (default: %u)", MAX_OP_RETURN_RELAY), ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-relaynonstandardblocks", "Relay blocks containing transactions that are not standard for transaction relay (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (args.IsArgSet("-bootstrap")) {
        const std::string bootstrap_peer = args.GetArg("-bootstrap", "");
        if (bootstrap_peer.empty()) {
            return InitError(_("Invalid -bootstrap value (must be a non-empty peer address)."));
        }
    }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    std::string par_arg = args.GetArg("-par", std::to_string(DEFAULT_SCRIPTCHECK_THREADS));
    int script_threads = ComputeScriptCheckThreadsFromParValue(par_arg);
    ...
    fDiscoverDefaultOnly = false;
    if (args.IsArgSet("-discover")) {
        const std::string discover_mode = ToLower(args.GetArg("-discover", ""));
        if (discover_mode == "defaultonly") {
            fDiscover = true;
            fDiscoverDefaultOnly = true;
        }
    }
    ...
    if (args.IsArgSet("-bootstrap")) {
        const bool bootstrap_needed = WITH_LOCK(::cs_main, return chainman.IsBootstrapChainstatePending());
        const std::string bootstrap_peer = args.GetArg("-bootstrap", "");
        if (bootstrap_needed && !bootstrap_peer.empty()) {
            if (node.connman->AddNode(bootstrap_peer)) {
                LogPrintf("Bootstrap mode active: added trusted bootstrap peer %s\n", bootstrap_peer);
            } else {
                LogPrintf("Bootstrap mode active, but failed to add trusted bootstrap peer %s\n", bootstrap_peer);
            }
        }
    }
    ...
    // If pruning, perform the initial blockstore prune after any wallet
    // rescanning has taken place.
    if (fPruneMode) {
        if (!fReindex) {
            LOCK(cs_main);
    ...
    connOptions.m_max_outbound_block_relay = std::clamp<int>(
        args.GetIntArg("-blockrelaypeers", MAX_BLOCK_RELAY_ONLY_CONNECTIONS),
        0, std::max(0, connOptions.nMaxConnections - connOptions.m_max_outbound_full_relay));
    ...
    if (args.GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)) {
        if (connOptions.onion_binds.size() > 1) {
            LogPrintf("Multiple onion bind addresses are provided. Each will be used to create a separate Tor onion service.\n");
        }
