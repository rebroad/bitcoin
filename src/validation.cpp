...
private:
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state, NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        AssertLockHeld(::cs_main);
        AssertLockHeld(m_pool.cs);
        // Allow zero-fee for wallet-originated transactions
        if (nodeid == NODEID_WALLET_ORIGIN) return true;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // blocks
    if (!bypass_limits && !CheckFeeRate(ws.m_vsize, ws.m_modified_fees, state, args.m_nodeid)) return false;
    ...
    do {
        ...
        do {
            ...
            if (IsGuiVisible()) {
                ...
                // Sleep in 1ms chunks, checking for GUI activity and idle time
                ...
                while (true) {
                    ...
                    auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_check_time).count();
                    ...
                    if (last_used > last_activity_time) {
                        ...
                        if (gui_was_active) {
                            if (elapsed_ms > longest_idle_gap_ms) {
                                longest_idle_gap_ms = elapsed_ms;
                            }
                        }

                        LogPrint(BCLog::QT, "ActivateBestChain: GUI activity%s detected %dms idle\n",
                                gui_was_active ? "" : " (initial)", elapsed_ms);
                        gui_was_active = true;
                    ...
                    auto idle_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_used).count();
...
MempoolAcceptResult ChainstateManager::ProcessTransaction(const CTransactionRef& tx, NodeId nodeid, bool test_accept)
{
    AssertLockHeld(cs_main);
    ...
    auto result = AcceptToMemoryPool(active_chainstate, tx, GetTime(), nodeid, /*bypass_limits=*/ false, test_accept);
