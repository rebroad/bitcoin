...
void LimitMempoolSize(CTxMemPool& pool, size_t limit, unsigned long age) {
    ...
    if (expired != 0)
        LogPrint("tx", "Expired %i transactions from the memory pool\n", expired);
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < ::minRelayTxFee.GetFee(nSize))
        {
            ...
            LogPrint("tx", "Rate limit dFreeCount: %g => %g\n", dFreeCount, dFreeCount+nSize);
            dFreeCount += nSize;
        }
        ...
        {
            LogPrint("tx", "replacing tx %s with %s for %s BTC additional fees, %d delta bytes\n",
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s (%d) ver=0x%x age=%s work=%.8g behind=%d tx=%lu\n", __func__,
...
int ProcessNewHeaders(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex)
{
    int nCount = 0;
    int ret;
    {
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    LogPrintf("%s: sorting vSortedByHeight\n", __func__);
    ...
    LogPrintf("%s: hashBestChain=%s height=%d age=%s progress=%f\n", __func__,
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(),
        strAge(GetTime()-chainActive.Tip()->GetBlockTime()),
...
bool LoadMempool(void)
{
    ...
    try {
        ...
        if (it == mapData.end()) {
            try {
                CDataStream ss(it->second, SER_NETWORK, PROTOCOL_VERSION);
        ...
        if (it == mapData.end()) {
            ...
            try {
                CDataStream(it->second, SER_NETWORK, PROTOCOL_VERSION) >> txMapDatas;
            } catch (const std::exception& e) {
                LogPrintf("Failed to deserialize mempool %s from disk: %s. Continuing anyway.\n", "transactions", e.what());
            }
            for (auto mapTxData : txMapDatas) {
                try {
                    it = mapTxData.find("t");
                    if (it == mapTxData.end()) {
                        throw runtime_error("mapTxData \"t\" key missing");
                    }
                    int64_t nTime;
                    CDataStream(it->second, SER_NETWORK, PROTOCOL_VERSION) >> nTime;
                    ...
                    it = mapTxData.find("");
                    if (it == mapTxData.end()) {
                        throw runtime_error("mapTxData null key missing");
                    }
                    CDataStream ssTx(it->second, SER_NETWORK, PROTOCOL_VERSION);
                    CTransaction tx(deserialize, ssTx);

                    CValidationState state;
                    LOCK(cs_main);
                    AcceptToMemoryPoolWithTime(mempool, state, MakeTransactionRef(tx), true, NULL, nTime);
                    if (!state.IsValid()) {
