...

    {
        ...
        if (auto mempool_tx = node.mempool->get(txid); mempool_tx) {
            ...
            if (max_tx_fee > 0) {
                ...
                const MempoolAcceptResult result = node.chainman->ProcessTransaction(tx, -1, /*test_accept=*/ true);
            ...
            const MempoolAcceptResult result = node.chainman->ProcessTransaction(tx, -1, /*test_accept=*/ false);
            if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
                return HandleATMPError(result.m_state, err_string);
            }
