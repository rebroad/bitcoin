...
TransactionError BroadcastTransaction(NodeContext& node, const CTransactionRef tx, std::string& err_string, const CAmount& max_tx_fee, bool relay, bool wait_callback, NodeId nodeid)
{
...

    {
        ...
        if (auto mempool_tx = node.mempool->get(txid); mempool_tx) {
            ...
            if (max_tx_fee > 0) {
                ...
                const MempoolAcceptResult result = node.chainman->ProcessTransaction(tx, nodeid, /*test_accept=*/ true);
            ...
            const MempoolAcceptResult result = node.chainman->ProcessTransaction(tx, nodeid, /*test_accept=*/ false);
            if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
                return HandleATMPError(result.m_state, err_string);
            }
