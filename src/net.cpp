...
// Run the feeler connection loop once every 5 seconds.
static const int FEELER_INTERVAL = 5;
...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        if (addrLocal.IsRoutable()) {
            LogPrint("net2", "%s: advertising local routable address %s to peer=%d\n", __func__, addrLocal.ToString(), pnode->id);
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fFeeler)
{
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, Params().GetDefaultPort(), nConnectTimeout, fFeeler, &proxyConnectionFailed) :
                  ConnectSocket(addrConnect, hSocket, nConnectTimeout, fFeeler, &proxyConnectionFailed))
    {
        ...
        CNode* pnode = new CNode(id, nLocalServices, GetBestHeight(), hSocket, addrConnect, CalculateKeyedNetGroup(addrConnect), nonce, pszDest ? pszDest : "", false, fFeeler);
...
bool CNode::ReceiveMsgBytes(const char *pch, unsigned int nBytes, bool& complete, int64_t& tLastRecvBlk)
{
    ...
    while (nBytes > 0) {
        ...
        std::string pchCommand;
        if (msg.in_data) {
            pchCommand = msg.hdr.pchCommand;
            if (pchCommand == NetMsgType::BLOCK || pchCommand == NetMsgType::CMPCTBLOCK || pchCommand == NetMsgType::BLOCKTXN || pchCommand == NetMsgType::HEADERS) {
                ...
                if (msg.complete()) {
                    ...
                    tLastBlkRep = nNow;
                    LogPrint("partial", "net Incoming %s (%u%% of %u bytes) chk=%s peer=%d\n", pchCommand, msg.hdr.nMessageSize ? (msg.nDataPos * 100 / msg.hdr.nMessageSize) : 100, msg.hdr.nMessageSize, strChk, id);
                }
            }
        }
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        if (nBytes > 0) {
            ...
            if (nBytes < 0) {
                ...
                {
                    LogPrint("net", "socket send error=%s, fDisconnect=%d TimeConn=%s peer=%d\n", NetworkErrorString(nErr), pnode->fDisconnect, strAge(GetTime() - pnode->nTimeConnected), pnode->id);
...
void CConnman::WakeMessageHandler()
{
    ...
    //if (fMsgProcSleep)
    //    LogPrint("timings", "%s: condMsgProc.notify_one()\n", __func__);
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        //
        bool fFeeler = false;
        if (nOutboundRelevant >= nMaxOutbound) { // don't count fDisconnected nodes
...
void CConnman::ThreadMessageHandler()
{
    ...
    while (!flagInterruptMsgProc || nBlocksToBeProcessed > 0)
    {
        ...
        tAfter = GetTimeMillis();
        if (tAfter > tBefore+1)
            strDurations += strprintf("SendRecv=%dms ", tAfter - tBefore);
        tBefore = tAfter;
        ...
        if (nDuration > 0 || tAfter-tBefore < 10000)
            LogPrint("timings", "%s: %s=%dms Slept=%dms nMoreWork=%d nPaused=%d\n", __func__, nDuration > 1000 ? "DURATION" : "Duration", nDuration, tAfter - tBefore, nMoreWork, nPaused); // REBTEMP
        //if (nDuration > 1000)
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    // Validate blocks
    threadValidation = std::thread(&TraceThread<boost::function<void()> >, "validate", std::function<void()>(std::bind(&CConnman::ThreadValidation, this)));
...
CNode::CNode(NodeId idIn, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, SOCKET hSocketIn, const CAddress& addrIn, uint64_t nKeyedNetGroupIn, uint64_t nLocalHostNonceIn, const std::string& addrNameIn, bool fInboundIn, bool fFeelerIn) :
