...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        {
            LogPrint(BCLog::NET, "%s: advertising local routable address %s to peer=%d\n",
                __func__, addrLocal.ToString(), pnode->GetId());
...
void CConnman::ThreadSocketHandler()
{
    ...
    while (!interruptNet)
    {
        ...
        if(vNodesSize != nPrevNodeCount) {
            ...
            if (!vNodes.size())
                LogPrint(BCLog::NET, "NO PEERS CONNECTED.\n"); // REBTODO - reset NodeId?
        }
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        if (addrConnect.IsValid()) {
            ...
            bool fRet = OpenNetworkConnection(addrConnect, (int)setConnected.size() >= std::min(nMaxConnections - 1, 2), &grant, nullptr, false, fFeeler);
