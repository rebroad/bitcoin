    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                {
                    LogPrint("net", "peer=%d socket no message in first 60 seconds, %d %d\n", pnode->id, pnode->nLastRecv != 0, pnode->nLastSend != 0);
                    pnode->fDisconnect = true;
                }
                else if (nTime - pnode->nLastSend > TIMEOUT_INTERVAL)
                {
                    LogPrintf("peer=%d socket sending timeout: %is\n", pnode->id, nTime - pnode->nLastSend);
                    pnode->fDisconnect = true;
                }
                else if (nTime - pnode->nLastRecv > (pnode->nVersion > BIP0031_VERSION ? TIMEOUT_INTERVAL : 90*60))
                {
                    LogPrintf("peer=%d socket receive timeout: %is\n", pnode->id, nTime - pnode->nLastRecv);
                    pnode->fDisconnect = true;
                }
...
void CNode::EndMessage() UNLOCK_FUNCTION(cs_vSend)
{
    ...
    {
        LogPrint("net2", "dropmessages DROPPING SEND MESSAGE\n");
        AbortMessage();
        return;
    }
