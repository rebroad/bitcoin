...
bool PeerTableSortProxy::lessThan(const QModelIndex& left_index, const QModelIndex& right_index) const
{
    ...
    case PeerTableModel::Recv: {
        int64_t now = GetTimeSeconds();
        int Right = right_stats.nRecvBytes * 8 / (now + 1 - count_seconds(right_stats.m_connected));
        int Left = left_stats.nRecvBytes * 8 / (now + 1 - count_seconds(left_stats.m_connected));
        return Left < Right;
    }
