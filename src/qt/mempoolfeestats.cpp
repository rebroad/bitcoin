...
void MempoolFeeStats::drawChart()
{
    ...
    qreal maxwidth = m_gfx_view->scene()->sceneRect().width()-GRAPH_PADDING_LEFT-GRAPH_PADDING_RIGHT;
    {
        ...
        */
        ...
        //ile >> m_clientmodel->m_mempool_feehist;
        //file.fclose();
        ...
        if (max_totalsize > 0) {
            int val = floor(log10(1.0*max_totalsize/amount_of_h_lines));
            int stepbase = pow(10.0f, val);
            int step = ceil((1.0*max_totalsize/amount_of_h_lines) / stepbase) * stepbase;
