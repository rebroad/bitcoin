...
static bool WriteKeyFile(const fs::path& outdir, const std::string& service_id, const std::string& key_b64) {
...
int main(int argc, char** argv) {
    ...
    auto worker_fn = [&](uint32_t) {
        ...
        while (!g_stop.load()) {
            ...
            std::string service_id = OnionServiceIdFromPubkey(pk);
            bool hit = false;
            ...
            sodium_bin2base64(b64, sizeof(b64), sk, crypto_sign_SECRETKEYBYTES, sodium_base64_VARIANT_ORIGINAL);
            bool wrote = WriteKeyFile(opts.outdir, service_id, b64);
            if (wrote) {
