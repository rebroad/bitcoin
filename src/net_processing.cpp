...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrint("block", "recv getblocktxn %s not found. peer=%d", req.blockhash.ToString(), pfrom->id);
            return true;
        }
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!pfrom->vRecvGetData.empty()) return true;
        ...
        LogPrint("netrecv", "receive: %s %d%% chk=%s vProcessMsg.size=%d complete=%d peer=%d\n", strCommand, msg.hdr.nMessageSize ? (msg.nDataPos * 100 / msg.hdr.nMessageSize) : 100, msg.hdr.nMessageSize, strChk, pfrom->vProcessMsg.size(), msg.complete(), pfrom->id);
