...
void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    bool fRelay = pnode->fFeeler ? false : ::fRelayTxes; // Feeler connections don't want TXs
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    LogPrint(BCLog::BLOCK, "recv getdata %s - UNKNOWN! peer=%d\n", inv.ToString(), pfrom->GetId());
                ...
                {
                    LogPrint(BCLog::BLOCK, "recv getdata %s. historical block serving limit reached, disconnect peer=%d\n", strBlockInfo(mi->second), pfrom->GetId());
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    LogPrint(fRecent ? BCLog::BLOCK : BCLog::BLOCKHIST, "recv getdata %s - sending. peer=%d\n", strBlockInfo(mi->second), pfrom->GetId());
                    ...
                    LogPrint(fRecent ? BCLog::BLOCK : BCLog::BLOCKHIST, "recv getdata %s %s - PRUNED. peer=%d\n", inv.ToString(), strBlkInfo(mi->second), pfrom->GetId());
            }
...
inline void static SendBlockTransactions(const CBlock& block, const BlockTransactionsRequest& req, CNode* pfrom, CConnman* connman) {
    ...
    for (size_t i = 0; i < req.indexes.size(); i++) {
        if (req.indexes[i] >= block.vtx.size()) {
            LOCK(cs_main);
            LogPrintf("recv getblocktxn. too many indexes (%d > %d) peer=%d\n", req.indexes[i], block.vtx.size(), pfrom->GetId());
...
void LogRecv(int nNew, const CBlockIndex *pindex, std::string strType, int nSize, int node)
{
    ...
    else if (pindex && pindex->nChainWork < chainActive.Tip()->nChainWork)
        strDesc += "old "; // it's behind our current tip
    else if (pindex && pindex->nTx > 0)
        strDesc += "got "; // it's been downloaded
    std::string strSize;
    if (nSize)
        strSize = strprintf("size=%d ", nSize);
    bool fRecent = false;
    if (pindex && pindex->nChainWork >= (pindexBestHeader->pprev ? (pindexBestHeader->pprev->pprev ? pindexBestHeader->pprev->pprev->nChainWork : 0) : 0))
        fRecent = true;
    LogPrint((nNew || fRecent) ? BCLog::BLOCK : BCLog::BLOCKHIST, "recv %s%s%s %s %speer=%d\n", strNew, strDesc, strType, strBlockInfo(pindex), strSize, node);
}
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    int nNew = ProcessNewHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header);
    if (nNew < 0) {
        int nDoS;
        if (state.IsInvalid(nDoS)) {
            LOCK(cs_main);
            std::string strBlock = strprintf(" %s", strBlockInfo(pindexLast));
            if (nDoS > 0) {
                LogPrintf("recv header%s not accepted peer=%d\n", strBlock, pfrom->GetId());
                Misbehaving(pfrom->GetId(), nDoS);
            } else
                LogPrint(BCLog::BLOCK, "recv header%s not accepted peer=%d\n", strBlock, pfrom->GetId());
...

    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            LogPrint(BCLog::BLOCK, "more getheaders (%d) to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom->GetId(), pfrom->nStartingHeight);
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                LogPrint(BCLog::BLOCK, "Large reorg, won't direct fetch to %s\n",
                ...
                if (vGetData.size() > 1) {
                    LogPrint(BCLog::BLOCK, "Requesting %d blocks toward %s (%d) from peer=%d\n",
                            vGetData.size(), pindexLast->GetBlockHash().ToString(), pindexLast->nHeight,
                            pfrom->GetId());
                }
                if (vGetData.size() > 0) {
                    ...
                    if (vGetData.size() == 1) // as was not logged above
                            LogPrint(BCLog::BLOCK, "Requesting %s %s from peer=%d\n",
                                vGetData[0].ToString(), strBlkInfo(vToFetch[0]), pfrom->GetId());
...
{
    LogPrint(BCLog::NET2, "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->GetId());
    ...
    if (strCommand == NetMsgType::REJECT)
    {
            try {
                ...
                if (strMsg == NetMsgType::BLOCK)
                    LogPrint(BCLog::BLOCK, "recv reject %s peer=%d\n", SanitizeString(ss.str()), pfrom->GetId());
                else if (strMsg == NetMsgType::TX)
                    LogPrint(BCLog::TX, "recv reject %s peer=%d\n", SanitizeString(ss.str()), pfrom->GetId());
                else
                    LogPrint(BCLog::NET, "recv reject %s peer=%d\n", SanitizeString(ss.str()), pfrom->GetId());
            } catch (const std::ios_base::failure&) {
                // Avoid feedback loops by preventing reject messages from triggering a new reject message.
                LogPrint(BCLog::NET, "recv reject. unable to parse. peer=%d\n", pfrom->GetId());
            }
    }
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        {
            LogPrint(BCLog::NET, "recv version does not offer the expected services (%08x offered, %08x expected); disconnecting peer=%d\n", nServices, pfrom->nServicesExpected, pfrom->GetId());
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (fListen && !IsInitialBlockDownload())
            {
                ...
                if (addr.IsRoutable())
                {
                    LogPrint(BCLog::NET, "ProcessMessages: advertising routable address %s to peer=%d\n", addr.ToString(), pfrom->GetId());
                    pfrom->PushAddress(addr, insecure_rand);
                } else if (IsPeerAddrLocalGood(pfrom)) {
                    addr.SetIP(addrMe);
                    LogPrint(BCLog::NET, "ProcessMessages: advertising local address %s to peer=%d\n", addr.ToString(), pfrom->GetId());
                    pfrom->PushAddress(addr, insecure_rand);
                }
            }
        ...
        LogPrintf("recv version %s version=%d, blocks=%d, us=%s%s, peer=%d\n",
                  ...
                  pfrom->nStartingHeight, addrMe.ToString(), remoteAddr, pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        connman->AddNewAddresses(vAddrOk, pfrom->addr, pfrom->GetId(), 2 * 60 * 60);
        if (vAddr.size() < 1000)
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (it == mapBlockIndex.end()) {
                    ...
                    LogRecv(0, it->second, "ïnv block", 0, pfrom->GetId());
                }
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint(BCLog::BLOCK, "send getheaders (%d) %s peer=%d\n", pindexBestHeader->nHeight, inv.hash.ToString(), pfrom->GetId());
                }
            }
            else if (inv.type == MSG_TX) {
                fAlreadyHave = AlreadyHave(inv);
                inv.type |= nFetchFlags;
                LogPrint(BCLog::TX, "recv inv %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", pfrom->GetId());
                pfrom->AddInventoryKnown(inv);
                if (fBlocksOnly) {
                    LogPrint(BCLog::TX, "recv inv %s in violation of protocol peer=%d\n", inv.hash.ToString(), pfrom->GetId());
                } else if (!fAlreadyHave && !fImporting && !fReindex && !IsInitialBlockDownload()) {
                    pfrom->AskFor(inv);
                }
            }
            else
                LogPrint(BCLog:: NET, "recv inv: %s  %s peer=%d\n", inv.ToString(), AlreadyHave(inv) ? "have" : "new", pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        int nLimit = 500;
        LogPrint(BCLog::BLOCK, "recv getblocks %d to %s limit %d peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                LogPrint(BCLog::BLOCK, "  getblocks stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            {
                LogPrint(BCLog::BLOCK, " getblocks stopping, pruned or too old block at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (--nLimit <= 0)
            {
                ...
                LogPrint(BCLog::BLOCK, "  getblocks stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end()) {
            LogPrint(BCLog::BLOCK, "recv getblocktxn %s. not seen. peer=%d\n", req.blockhash.ToString(), pfrom->GetId());
            return true;
        } else if (!(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrint(BCLog::BLOCK, "recv getblocktxn %s. not have. peer=%d\n", strBlockInfo(it->second), pfrom->GetId());
            return true;
        }
        ...
        if (it->second->nChainWork < chainActive[std::max(0, chainActive.Height() - MAX_BLOCKTXN_DEPTH)]->nChainWork) {
            ...
            LogPrint(BCLog::BLOCK, "recv getblocktxn %s too deep (> %i) peer=%d", strBlockInfo(it->second), MAX_BLOCKTXN_DEPTH, pfrom->GetId());
            CInv inv;
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        if (IsInitialBlockDownload() && !pfrom->fWhitelisted) {
            LogPrint(BCLog::BLOCK, "recv getheaders. Ignoring because node is in initial block download. peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        LogPrint(BCLog::BLOCK, "recv getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        {
            LogPrint(BCLog::TX, "transaction sent in violation of protocol peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        if (state.IsInvalid(nDoS))
        {
            LogPrint(BCLog::MEMPOOL, "%s from peer=%d was not accepted: %s\n", tx.GetHash().ToString(),
                pfrom->GetId(),
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (nNew < 0) {
            int nDoS;
            if (state.IsInvalid(nDoS)) {
                std::string strBlock = strprintf(" %s", strBlockInfo(pindex));
                if (nDoS > 0) {
                    LOCK(cs_main);
                    LogPrintf("recv cmpctblock%s size=%d INVALID HEADER peer=%d\n", strBlock, nSize, pfrom->GetId());
                    Misbehaving(pfrom->GetId(), nDoS);
                } else
                    LogPrint(BCLog::BLOCK, "recv cmpctblock%s size=%d INVALID HEADER peer=%d\n", strBlock, nSize, pfrom->GetId());
                return true;
            }
        }
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrint(BCLog::BLOCK, "cmpctblock %s already being synched! peer=%s\n",
                ...
                if (status == READ_STATUS_INVALID) {
                    ...
                    vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(pfrom), cmpctblock.header.GetHash());
                    connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETDATA, vInv));
                    LogPrint(BCLog::BLOCK, "cmpctblock %s failed. Requesting block. peer=%d\n",
                        strBlockInfo(pindex), pfrom->GetId());
                    return true;
                }
            ...
            if (fAlreadyInFlight) {
                ...
                vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(pfrom), cmpctblock.header.GetHash());
                connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETDATA, vInv));
                LogPrint(BCLog::BLOCK, "cmpctblock %s too far ahead of tip. Request block. peer=%d\n",
                    strBlockInfo(pindex), pfrom->GetId());
                return true;
            } else {
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (it == mapBlocksInFlight.end()) {
                LogPrint(BCLog::BLOCK, "recv blocktxn %s not known. peer=%d\n", resp.blockhash.ToString(), pfrom->GetId());
                return true;
            } else if (!it->second.second->partialBlock) {
                LogPrint(BCLog::BLOCK, "recv blocktxn %s %s not requested. peer=%d\n", resp.blockhash.ToString(), strBlkInfo(it->second.second->pindex), pfrom->GetId());
                return true;
            }
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                if (it->second.first == pfrom->GetId()) { // Only misbehaving if it's the one we expected
                    LogPrintf("recv blocktxn %s %s. INVALID! peer=%d\n", resp.blockhash.ToString(), strBlkInfo(it->second.second->pindex), pfrom->GetId());
                    Misbehaving(pfrom->GetId(), 100);
                }
                return true;
            } else if (status == READ_STATUS_FAILED) {
                ...
                LogPrint(BCLog::BLOCK, "recv blocktxn %s %s FAILED. send getdata block peer=%d\n", resp.blockhash.ToString(), strBlkInfo(it->second.second->pindex), pfrom->GetId());
            } else {
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint(BCLog::BLOCK, "recv block %s%s peer=%d\n", pblock->GetHash().ToString(), strExtra, pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrint(BCLog::NET, "recv feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), pfrom->GetId());
        }
    }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted < 2 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                LogPrint(BCLog::BLOCK, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->GetId(), pto->nStartingHeight);
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    LogPrint(BCLog::BLOCK, "%s sending header-and-ids %s to peer=%d\n", __func__,
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrint(BCLog::BLOCK, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogPrint(BCLog::BLOCK, "%s: sending header %s to peer=%d\n", __func__,
                                vHeaders.front().GetHash().ToString(), pto->GetId());
                    }
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    // Just log for now.
                    if (chainActive[pindex->nHeight] != pindex) {
                        LogPrint(BCLog::BLOCK, "Announcing block %s (%s) not on main chain (tip=%s)\n",
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            if (!AlreadyHave(inv))
            {
                LogPrint(BCLog::TX, "Requesting %s peer=%d\n", inv.ToString(), pto->GetId());
