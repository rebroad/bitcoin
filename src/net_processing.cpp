        ...
        m_connman.ForNode(nodeid, [this](CNode* pfrom) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
                ...
                m_connman.ForNode(lNodesAnnouncingHeaderAndIDs.front(), [this, nCMPCTBLOCKVersion](CNode* pnodeStop){
                    LogPrint(BCLog::BLOCK, "send sendcmpct (no announce) ver=%d peer=%d\n", nCMPCTBLOCKVersion, pnodeStop->GetId());
            ...
            LogPrint(BCLog::BLOCK, "send sendcmpct (announce) ver=%d peer=%d\n", nCMPCTBLOCKVersion, pfrom->GetId());
...
std::string PeerManagerImpl::strBlkInfo(const CBlockIndex* pindex, bool* fFork /*=nullptr*/)
{
    ...
    const bool is_ibd = m_chainman.ActiveChainstate().IsInitialBlockDownload();
    const std::string ahead_behind = nAhead > 0 ? strprintf(" ahead=%d", nAhead) :
                                    (is_ibd && nBehind > 0) ? strprintf(" behind=%d", nBehind) : "";
...
bool PeerManagerImpl::IsBlockInFlight(const uint256& block_hash) const
{
    LOCK(cs_main);
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    m_connman.ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        AssertLockHeld(::cs_main);
...
void PeerManagerImpl::BlockChecked(const CBlock& block, const BlockValidationState& state)
{
    ...
    //    just check that there are currently no other blocks in flight.
        ...
        const CBlockIndex* pindex = m_chainman.m_blockman.LookupBlockIndex(hash);
        const bool is_historical_backfill = pindex && pindex->nHeight < m_chainman.ActiveChain().Height();
        if (it != mapBlockSource.end() && !is_historical_backfill) {
                ...
                if (bytes > 0 || txs > 0) {
                    ...
                    m_connman.ForNode(nodeid, [this, bytes, txs, now, use_peer_stats_snapshots](CNode* pnode) {
                        ...
                        if (nodestate) {
                            ...
                            const uint64_t send_bytes{pnode->nSendBytes > send_bytes_base ? pnode->nSendBytes - send_bytes_base : 0};
                            ...
                            pnode->nBTxBpsPct = (recv_bytes + send_bytes) > 0 ? 100.0 * block_bytes / (recv_bytes + send_bytes) : 0;
                            pnode->nBTXpm = now > time_base ? 60.0 * block_txs / (now - time_base) : 0;
                            LogPrint(BCLog::BLOCK, "peer=%d, recv=%d Pct = (%d-%d) / (%d+%d) = %d\n",
                                    pnode->GetId(), nodestate->nBlocksRecv, pnode->nBlockBytes, pnode->nBlockBytesSnapOld,
                                    pnode->nRecvBytes - pnode->nRecvBytesSnapOld, pnode->nSendBytes - pnode->nSendBytesSnapOld, pnode->nBTxBpsPct);
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (it != peer.m_getdata_requests.end() && !pfrom.fPauseSend) {
        const CInv &inv = *it++;
        if (inv.IsGenBlkMsg() && fDownloadBlocks) {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::SENDRECON) {
        ...
        if (!pfrom.m_tx_relay) {
            ...
            // want to reconcile.
            LogPrintf("sendrecon received from non-tx-relay. disconnecting  peer=%d\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (!State(pfrom.GetId())->m_wtxid_relay) {
            ...
            LogPrintf("sendrecon received before wtxidrelay. disconnecting peer=%d\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (!m_reconciliation.RegisterPeer(pfrom.GetId(), pfrom.IsInboundConn(),
            they_initiator, they_responder, recon_version, remote_salt)) {
                LogPrintf("reconciliation protocol violation. disconnecting peer=%d\n", pfrom.GetId());
                pfrom.fDisconnect = true;
                return;
            }
        return;
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        if (pfrom.IsAddrFetchConn() && vAddr.size() > 1) {
            LogPrintf("addrfetch connection completed. disconnecting peer=%d\n", pfrom.GetId());
            pfrom.fDisconnect = true;
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        if (best_block != nullptr) {
            ...
            LogPrint(BCLog::BLOCK, "getheaders (%d) %s to peer=%d\n", pindexBestHeader->nHeight, best_block->ToString(), pfrom.GetId());
        }
    ...
    if (msg_type == NetMsgType::GETBLOCKTXN) {
...

        {
            ...
            if (!pindex || !(pindex->nStatus & BLOCK_HAVE_DATA)) {
                LogPrintf("CURIOUS: recv getblocktxn for a block we don't have. peer=%d\n", pfrom.GetId());
                return;
            }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        LOCK(cs_main);
        CNodeState *nodestate = State(pfrom.GetId());
        if (nodestate->fPreferHeaders == false)
            LogPrintf("CURIOUS: headers requested yet prefers INVs. peer=%d\n", pfrom.GetId());
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
...

        {
            ...
            if (!m_chainman.m_blockman.LookupBlockIndex(cmpctblock.header.hashPrevBlock)) {
                ...
                if (!m_chainman.ActiveChainstate().IsInitialBlockDownload()) {
                    m_connman.PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETHEADERS, m_chainman.ActiveChain().GetLocator(pindexBestHeader), uint256()));
                    LogPrint(BCLog::BLOCK, "recv cmpctblock %s send getheaders (%s) peer=%d\n", stripZeros(cmpctblock.header.GetHash().ToString()), strHeight(pindexBestHeader), pfrom.GetId());
                }
                return;
            }
        }
        ...
        if (!fUpdateChain || pindex->nHeight <= m_chainman.ActiveChain().Height() + 6) { // REBTODO - do extra checks here relating to age
            ...
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!fSeenBefore) LogPrint(BCLog::BLOCK, "TX have from%s\n", strTXfrom);
        ...
        if (fRevertToHeaderProcessing) {
            LogPrint(BCLog::BLOCK, "Calling ProcessHeadersMessage(cmpctblock.header) peer=%d\n", pfrom.GetId());
    ...
    if (msg_type == NetMsgType::BLOCKTXN)
    {
        ...
        bool fWrongPeer = false;
        {
            ...
            if (it == mapBlocksInFlight.end()) {
                LogPrint(BCLog::BLOCK, "recv blocktxn %s not in flight. peer=%d\n", resp.blockhash.ToString(), pfrom.GetId());
                return;
            }
            ...
            } else if (status == READ_STATUS_FAILED && !fWrongPeer && fDownloadBlocks) {
    ...
    if (now > m_stale_tip_check_time) {
        ...
        if (!fImporting && !fReindex && m_connman.GetNetworkActive() && m_connman.GetUseAddrmanOutgoing() && TipMayBeStale()) {
            LogPrintf("Potential stale tip detected, will try using extra outbound peer (last tip update: %s ago)\n",
                      strAge(count_seconds(now - m_last_tip_update.load())));
...
void PeerManagerImpl::MaybeSendAddr(CNode& node, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (fListen && !m_chainman.ActiveChainstate().IsInitialBlockDownload() &&
        ...
        peer.m_next_local_addr_send = GetExponentialRand(current_time, AVG_LOCAL_ADDRESS_BROADCAST_INTERVAL); // REBTODO - reduce interval for whitelisted bitnodes
    }
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrint(BCLog::BLOCKSEND, "send %u headers, range (%s, %s), to peer=%d\n",
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogPrint(BCLog::BLOCKSEND, "send header %s peer=%d\n",
            ...
            if (fRevertToInv) {
                ...
                if (!peer->m_blocks_for_headers_relay.empty()) {
                    ...
                    // Just log for now.
                    if (m_chainman.ActiveChain()[pindex->nHeight] != pindex) {
                        LogPrint(BCLog::BLOCKSEND, "send block inv %s not on main chain. peer=%d\n",
                            strBlockInfo(pindex), pto->GetId());
                    }
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrint(BCLog::BLOCKSEND, "send block inv %s peer=%d\n",
                            strBlockInfo(pindex), pto->GetId());
                    }
                }
            }
        ...
        if (fDownloadBlocks && !m_chainman.IsBootstrapChainstatePending() && !pto->fClient && ((fFetch && !pto->m_limited_node) || !m_chainman.ActiveChainstate().IsInitialBlockDownload()) && state.nBlocksInFlight < static_cast<int>(per_peer_inflight_limit)) {
            ...
            if (pto->IsBlockOnlyConn() && !pto->m_limited_node &&
...
void PeerManagerImpl::RotateNodeSnapshots(CNode* pnode, int64_t now) {
    ...
    pnode->nSendBytesSnapOld = pnode->nSendBytesSnap;
    pnode->nSendBytesSnap = pnode->nSendBytes;
...
{
    // Track stats only for compact blocks in the rolling buffer. This bounds
    // m_compact_block_stats and avoids stale entries for unknown hashes.
    bool hash_is_tracked{false};
    for (size_t i = 0; i < 3; ++i) {
        if (m_recent_compact_blocks[i].header.GetHash() == hash) {
            hash_is_tracked = true;
            break;
        }
    }
    if (!hash_is_tracked) return;
