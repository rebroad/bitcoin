...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    ...
    if (!state->fHaveWitness && state->pindexLastCommonBlock && IsWitnessEnabled(state->pindexLastCommonBlock, consensusParams)) {
        ...
        LogPrintf("blockblock", "UNBLOCKED - Witness now provided or not needed. peer=%d\n", nodeid);
        state->nBlockPaused = -6;
    }
            ...
            if (pindex->nStatus & BLOCK_HAVE_DATA || chainActive.Contains(pindex)) {
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    if (state->nBlockPaused != 7) {
                        LogPrint("blockblock", "BLOCKED - End of window. prevPause=%d nodeStaller=%d waitingfor=%d peer=%d\n", state->nBlockPaused, nodeStaller, waitingfor, nodeid);
                ...
                if (state->fExpectingBlocks && state->nMaxInFlight - count <= 0) {
                    if (GetBoolArg("-dynmax", false)) {
                        int nBlocksToBeProcessed = count - state->nMaxInFlight + state->nBlocksInFlight;
                        LogPrint("block", "%s: nMaxInFlight=%d 2bProc=%d InFlight=%d count=%d peer=%d\n", __func__, state->nBlocksInFlight, nBlocksToBeProcessed, state->nBlocksInFlight, count, nodeid);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < (int)nodestate->nMaxInFlight) || fAlreadyInFlight) {
    ...
    {
        if (fDebug) {
            std::string strMsg; std::string strReason;
...
bool SendMessages(CNode* pto, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (state.nStallingSince && state.nStallingSince < nNow - 1000000 * BLOCK_STALLING_TIMEOUT) {
            ...
            bool fFakeIt = (tNow - pto->tLastRecvBlk) < 60 && !GetBoolArg("-sipadisconnects", true);
