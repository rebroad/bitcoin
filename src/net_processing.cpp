...
void PeerManager::FinalizeNode(const CNode& node, bool& fUpdateConnectionTime) {
    ...
    int nErasedOrphans = EraseOrphansFor(nodeid);
    ...
    if (nBlocksInFlight)
        LogPrintf("%s: Lost %d blocks in flight, erased %d of %d of %d orphans. fDisconnect=%d LastRecv=%d LastSend=%d disconnecting peer=%d\n", __func__, nBlocksInFlight, nErasedOrphans, mapOrphanTransactions.size(), nMaxOrphans, node.fDisconnect ? 1:0, node.nLastRecv, node.nLastSend, nodeid);
    else if (nErasedOrphans)
        LogPrintf("%s: Erased %d of %d of %d orphans. fDisconnect=%d LastRecv=%d LastSend=%d disconnecting peer=%d\n", __func__, nErasedOrphans, mapOrphanTransactions.size(), nMaxOrphans, node.fDisconnect ? 1:0, node.nLastRecv, node.nLastSend, nodeid);
...
void PeerManager::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    if (!fInitialDownload) {
        ...
        m_connman.ForEachNode([nNewHeight, &vHashes, &strDebug](CNode* pnode) {
            LOCK(pnode->cs_inventory);
            if (nNewHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : 0)) {
                strDebug += strprintf(" %d", pnode->GetId());
...
void PeerManager::ProcessHeadersMessage(CNode& pfrom, const std::vector<CBlockHeader>& headers, bool via_compact_block)
{
    ...
    LogRecv(nNew, pindexLast, via_compact_block ? "cmpctblock" : "header", nSize, pfrom.GetId()); // REBTODO - can we deserialize to get the size?
    if (state.IsInvalid()) {
                                         ...
                                         const std::atomic<bool>& interruptMsgProc)
{
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (fBlocksOnly || pfrom.IsFeelerConn()) {
                    LogPrintf("recv inv tx violation HP=%d feel=%d grt=%d mtx=%d disconnecting peer=%d\n", pfrom.HasPermission(PF_RELAY) ? 1:0, pfrom.IsFeelerConn() ? 1:0, g_relay_txes ? 1:0, pfrom.m_tx_relay ? 1:0, pfrom.GetId());
                    pfrom.fDisconnect = true;
                    return;
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if ((!g_relay_txes && !pfrom.HasPermission(PF_RELAY)) || (pfrom.m_tx_relay == nullptr) || pfrom.IsFeelerConn())
        {
            LogPrintf("recv tx violation HP=%d feel=%d grt=%d mtx=%d disconnecting peer=%d\n", pfrom.HasPermission(PF_RELAY) ? 1:0, pfrom.IsFeelerConn() ? 1:0, g_relay_txes ? 1:0, pfrom.m_tx_relay ? 1:0, pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (!LookupBlockIndex(cmpctblock.header.hashPrevBlock)) {
            ...
            if (!::ChainstateActive().IsInitialBlockDownload())
                m_connman.PushMessage(&pfrom, msgMaker.Make(NetMsgType::GETHEADERS, ::ChainActive().GetLocator(pindexBestHeader), uint256()));
                LogPrint(BCLog::BLOCK, "recv cmpctblock %s send getheaders (%d) peer=%d\n", cmpctblock.header.GetHash().ToString(), strBlkHeight(pindexBestHeader), pfrom.GetId());
            return;
        }
        ...
        if (pindex->nHeight <= ::ChainActive().Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(m_mempool, pfrom.GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) { // REBTODO - Maybe don't do this if fWibble..?
    ...
    if (msg_type == NetMsgType::BLOCKTXN)
    {
        ...
        if (fBlockRead) {
            ...
            if (fWrongPeer)
                LogPrint(BCLog::BLOCK, "blocktxn Calling ProcessNewBlock() wrong peer=%d\n", pfrom.GetId());
            m_chainman.ProcessNewBlock(m_chainparams, pblock, /*fForceProcessing=*/true, &fNewBlock);
            if (fNewBlock) {
                pfrom.nLastBlockTime = GetTime();
            } else {
        ...
        if (pto->m_tx_relay != nullptr && pto->GetCommonVersion() >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            if (count_microseconds(current_time) > pto->m_tx_relay->nextSendTimeFeeFilter) {
                ...
                if ((filterToSend < pto->m_tx_relay->lastSentFeeFilter && currentFilter < pto->m_tx_relay->lastFeeFilter) ||
                    ...
                    LogPrintf("send feefilter %d->%d (actual: %d->%d) peer=%d\n", pto->m_tx_relay->lastSentFeeFilter, filterToSend, pto->m_tx_relay->lastFeeFilter, currentFilter, pto->GetId());
