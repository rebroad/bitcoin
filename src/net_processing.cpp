...
int64_t tSipaDisconnect = 0; // When sipa disconnect logic last invoked
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    ...
    if (count == 0) {
        if (state->nBlockPaused != 1) {
            ...
            LogPrint("block", "BLOCKED - count==0 peer=%d\n", nodeid);
        }
        return;
    } else if (state->nBlockPaused == 1) {
        LogPrint("block", "UNBLOCKED - count!=0 peer=%d\n", nodeid);
        state->nBlockPaused = -1;
    }
    ...
    if (!state->fHaveWitness && state->pindexLastCommonBlock && IsWitnessEnabled(state->pindexLastCommonBlock, consensusParams)) {
        ...
        if (state->nBlockPaused != 6) {
            ...
            LogPrint("block", "BLOCKED - Not providing witnesses. peer=%d\n", nodeid);
        }
        return;
    } else if (state->nBlockPaused == 6) {
        LogPrintf("block", "UNBLOCKED - Witness now provided or not needed. peer=%d\n", nodeid);
        state->nBlockPaused = -6;
    }
    ...
    if (state->pindexBestKnownBlock == NULL) {
        ...
        if (state->nBlockPaused != 2) {
            ...
            LogPrint("block", "BLOCKED - no headers received yet peer=%d\n", nodeid);
        }
        return;
    } else if (state->nBlockPaused == 2) {
        LogPrint("block", "UNBLOCKED - headers now received peer=%d\n", nodeid);
        state->nBlockPaused = -2;
    }
    ...
    if (state->pindexBestKnownBlock->nChainWork < chainActive.Tip()->nChainWork) {
        ...
        if (state->nBlockPaused != 3) {
            ...
            LogPrint("block", "BLOCKED - BestKnownBlockWork < TipWork peer=%d\n", nodeid);
        }
        return;
    } else if (state->nBlockPaused == 3) {
        LogPrint("block", "UNBLOCKED - BestKnownBlockWork >= TipWork peer=%d\n", nodeid);
        state->nBlockPaused = -3;
    }
    ...
    CBlockIndex *pindexPrevCommon = state->pindexLastCommonBlock;
        ...
        BOOST_FOREACH(CBlockIndex* pindex, vToFetch) {
            if (!pindex->IsValid(BLOCK_VALID_TREE)) {
                ...
                if (state->nBlockPaused != 5) {
                    ...
                    LogPrint("block", "BLOCKED - %s - chain is invalid. peer=%d\n", strBlockInfo(pindex), nodeid);
                }
                return;
            } else if (state->nBlockPaused == 5) {
                LogPrint("block", "UNBLOCKED - %s - chain is now valid. peer=%d\n", strBlockInfo(pindex), nodeid);
                state->nBlockPaused = -5;
            }
            if (!State(nodeid)->fHaveWitness && IsWitnessEnabled(pindex->pprev, consensusParams)) {
                ...
                if (state->nBlockPaused != 6) {
                    state->nBlockPaused = 6;
                    LogPrint("block", "BLOCKED - Witness just activated! Not providing witnesses. peer=%d\n", nodeid);
                }
                return;
            } else if (state->nBlockPaused == 6) {
                LogPrint("block", "UNBLOCKED - Witness de-activated! peer=%d\n", nodeid);
                state->nBlockPaused = -6;
            }
            if (pindex->nStatus & BLOCK_HAVE_DATA || chainActive.Contains(pindex)) {
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    if (state->nBlockPaused != 7) {
                        LogPrint("block", "BLOCKED - End of window. prevPause=%d nodeStaller=%d waitingfor=%d peer=%d\n", state->nBlockPaused, nodeStaller, waitingfor, nodeid);
                    ...
                    LogPrint("block", "UNBLOCKED - Height (%s) within window (%d) peer=%d\n", strHeight(pindex), nWindowEnd);
                    state->nBlockPaused = -7;
                }
...
void PeerLogicValidation::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    if (!fInitialDownload) {
        ...
        LogPrint("block", "PushBlockHash to");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrintf("recv getblocktxn %s not found. peer=%d", req.blockhash.ToString(), pfrom->id);
            return true;
        }
        ...
        if (it->second->nChainWork < chainActive[std::max(0, chainActive.Height() - MAX_BLOCKTXN_DEPTH)]->nChainWork) {
            ...
            LogPrint("block", "recv getblocktxn %s too deep (> %i) peer=%d", strBlockInfo(it->second), MAX_BLOCKTXN_DEPTH, pfrom->id);
            CInv inv;
        ...
        int nSendSize = connman.PushMessage(pfrom, msgMaker.Make(nSendFlags, NetMsgType::BLOCKTXN, resp));
            ...
            while (!vWorkQueue.empty()) {
                     ...
                     ++mi)
                {
                    ...
                    if (AcceptToMemoryPool(mempool, stateDummy, orphanTx, true, &fMissingInputs2)) {
                        LogPrint("tx", "   accepted orphan tx %s peer=%d\n", orphanHash.ToString(), fromPeer); // REBTODO - update stats for useful TXs (orphans that found parents) for this peer
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        BlockMap::iterator mi = mapBlockIndex.end();
        {
            LOCK(cs_main);
            ...
            if (mi->second->nTx > 0) {
                LogPrint("block", "recv blocktxn %s size=%d already downloaded. peer=%d\n", strBlockInfo(mi->second), nSize, pfrom->id);
                return true;
            }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nNew < 0) {
            ...
            if (state.IsInvalid(nDoS)) {
                std::string strBlock;
                if (pindexLast)
                    strBlock = strprintf(" %s %s", pindexLast->GetBlockHash().ToString(), strBlkInfo(pindexLast));
                else
                    strBlock = "s";
                if (nDoS > 0) {
        ...
        assert(pindexLast);
        CBlockIndex *pindexPrevBestKnown = nodestate->pindexBestKnownBlock;
...
bool SendMessages(CNode* pto, CConnman& connman)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    int nSize = connman.PushMessage(pto, msgMaker.Make(nSendFlags, NetMsgType::CMPCTBLOCK, cmpctblock));
        ...
        if (state.nStallingSince && state.nStallingSince < nNow - 1000000 * BLOCK_STALLING_TIMEOUT) {
            ...
            bool fFakeIt = (tNow - state.tLastRecvBlk) < 60 && !GetBoolArg("-sipadisconnects", true);
            if (!fFakeIt || tNow > state.tSipaDisconnect + 60) {
                LogPrintf("block download stalled. LastRecv=%is LastRecvBlk=%is LastSend=%is %sdisconnect peer=%d\n",
                        tNow-pto->nLastRecv, tNow-state.tLastRecvBlk, tNow-pto->nLastSend, fFakeIt ? "sipa " : "", pto->id);
                if (!fFakeIt) {
                    pto->fDisconnect = true;
                    return true;
                }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (nNow > state.nDownloadingSince + consensusParams.nPowTargetSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                ...
                if (!fFakeIt || (tNow > state.tSipaDisconnect + 60 * nOtherPeersWithValidatedDownloads && tNow > tSipaDisconnect + 60)) {
                    LogPrintf("block download timeout. DLingSince=%is LastRecv=%is LastRecvBlk=%is LastSend=%is %sdisconnect peer=%d\n", tNow-state.nDownloadingSince/1000000, tNow-pto->nLastRecv, tNow-state.tLastRecvBlk, tNow-pto->nLastSend, fFakeIt ? "sipa ": "", pto->id);
                    if (!fFakeIt) {
                        pto->fDisconnect = true;
                        return true;
                    }
        ...
        if ((fFetch || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            BOOST_FOREACH(CBlockIndex *pindex, vToDownload) {
                ...
                LogPrint("block", "send getdata block %s %s peer=%d\n", pindex->GetBlockHash().ToString(),
                    strBlkInfo(pindex), pto->id);
            }
