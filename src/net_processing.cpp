...
struct CNodeState {
    ...
    //! Whether we are expecting blocks
    bool fExpectingBlocks;
    //! Node is unable to download blocks for now
    int nBlockPaused;
    //! When sipa disconnect logic invoked
    int64_t tSipaDisconnect;
    //! Maximum blocks allowed in flight from this peer (times 10)
    ...
    CNodeState(CAddress addrIn, std::string addrNameIn) : address(addrIn), name(addrNameIn) {
        ...
        nStallingSince = 0;
        nDownloadingSince = 0;
        fExpectingBlocks = false;
        nBlockPaused = 0;
        tSipaDisconnect = 0;
        if (GetBoolArg("-dynmax", false))
            nMaxInFlight = 20; // Start at 2 (times 10)
        else
            nMaxInFlight = MAX_BLOCKS_IN_TRANSIT_PER_PEER * 10;
        nBlocksInFlight = 0;
...
bool MarkBlockAsReceived(const uint256& hash, NodeId nodeid) {
    ...
    if (!state->hashLastUnknownBlock.IsNull()) {
        ...
        if (itOld != mapBlockIndex.end() && itOld->second->nChainWork > 0) {
            if (state->pindexBestKnownBlock == NULL || itOld->second->nChainWork >= state->pindexBestKnownBlock->nChainWork) {
                const CBlockIndex *prevBestKnownBlock = state->pindexBestKnownBlock;
        ...
        connman.ForNode(nodeid, [&connman](CNode* pfrom){
            ...
            if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
                ...
                connman.ForNode(lNodesAnnouncingHeaderAndIDs.front(), [&connman, fAnnounceUsingCMPCTBLOCK, nCMPCTBLOCKVersion](CNode* pnodeStop){
                    LogPrint("block", "send sendcmpct (no announce) ver=%d peer=%d\n", nCMPCTBLOCKVersion, pnodeStop->id);
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    if (state->nBlockPaused != 7) {
                        ...
                        if (GetBoolArg("-dynmax", false) && waitingfor >= 0) {
                            ...
                            State(waitingfor)->nMaxInFlight-=1;
                        }
                    }
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    {
        LogPrint("tx", "ignoring large orphan tx (size: %u, hash: %s)\n", sz, hash.ToString());
        return false;
    }
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint("tx", "Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrint("tx", "Erased %d orphan tx due to expiration\n", nErased);
    }
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        if (state.fPreferHeaderAndIDs && (!fWitnessEnabled || state.fWantsCmpctWitness) &&
            ...
            LogPrint("net", "send cmpctblock %s peer=%d\n", strBlockInfo(pindex), pnode->id);
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION) {
            ...
            if (!pfrom->fInbound) {
                LogPrint(pfrom->fFeeler ? "feeler" : "net", "recv version obsolete %i; disconnecting outbound peer=%d\n", nVersion, pfrom->id);
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        connman.AddNewAddresses(vAddrOk, pfrom->addr, pfrom->id, 2 * 60 * 60);
        if (vAddr.size() < 1000)
    ...
    else if (strCommand == NetMsgType::SENDCMPCT)
    {
        ...
        if (fAnnounceUsingCMPCTBLOCK || State(pfrom->id)->fPreferHeaderAndIDs) // Don't show initial sendcmpct message
            LogPrint("block", "recv sendcmpct (%sannounce) ver=%d peer=%d\n", fAnnounceUsingCMPCTBLOCK ? "" : "no ", nCMPCTBLOCKVersion, pfrom->id);
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrint("block", "recv getblocktxn %s not found. peer=%d\n", req.blockhash.ToString(), pfrom->id);
            return true;
        }
        ...
        if (it->second->nChainWork < chainActive[std::max(0, chainActive.Height() - MAX_BLOCKTXN_DEPTH)]->nChainWork) {
            ...
            LogPrint("block", "recv getblocktxn %s too deep (> %i) peer=%d\n", strBlockInfo(it->second), MAX_BLOCKTXN_DEPTH, pfrom->id);
            CInv inv;
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        { // not accepted as missing inputs, i.e. an orphan
            ...
            if (!fRejectedParents) {
                ...
                if (nEvicted > 0)
                    LogPrint("tx", "mapOrphan overflow, removed %u tx\n", nEvicted);
            } else {
            ...
            if (pfrom->fWhitelisted && GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
                ...
                if (!state.IsInvalid(nDoS) || nDoS == 0) {
                    LogPrint("tx", "send tx %s force relaying whitelisted peer=%d\n", tx.GetHash().ToString(), pfrom->id);
                    RelayTransaction(tx, connman);
                } else {
                    LogPrint("tx", "Not relaying invalid tx %s (%s) whitelisted peer=%d (%s)\n", tx.GetHash().ToString(), FormatStateMessage(state), pfrom->id);
                }
            }
        }
        ...
        if (state.IsInvalid(nDoS))
        {
            ...
            if (nDoS > 0) {
                LogPrintf("recv tx %s not accepted (%s) peer=%d\n", tx.GetHash().ToString(), FormatStateMessage(state), pfrom->id);
                Misbehaving(pfrom->GetId(), nDoS);
            } else
                LogPrint("tx", "recv tx %s not accepted (%s) peer=%d\n", tx.GetHash().ToString(), FormatStateMessage(state), pfrom->id);
        }
    }
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fReindex) // Ignore blocks received while reindexing
    {
        ...
        if (mapBlockIndex.find(cmpctblock.header.hashPrevBlock) == mapBlockIndex.end()) {
            ...
            LogPrint("block", "recv cmpctblock %s. prev %s not found. peer=%d\n", cmpctblock.header.GetHash().ToString(), cmpctblock.header.hashPrevBlock.ToString(), pfrom->id);
        ...
        assert(pindex);
        LogRecv(nNew, pindex, "cmpctblock", nSize, pfrom->id);
        ...
        if (pindex->nChainWork <= chainActive.Tip()->nChainWork || // We know something better
            ...
            if (fAlreadyInFlight) {
                ...
                LogPrint("block", "resend getdata %s %s peer=%d\n", vInv[0].ToString(), strBlkInfo(pindex), pfrom->id);
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && (nodestate->nBlocksInFlight - pfrom->nBlocksToBeProcessed) < (int)nodestate->nMaxInFlight/10) || fAlreadyInFlight) {
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), chainparams.GetConsensus(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrint("block", "cmpctblock %s already being synced! peer=%d\n", strBlockInfo(pindex), pfrom->id);
                        return true;
                    }
                }
                ...
                if (status == READ_STATUS_INVALID) {
                    ...
                    std::vector<CInv> vInv(1);
                    vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(pfrom, pindex->pprev, chainparams.GetConsensus()), cmpctblock.header.GetHash());
                    LogPrint("block", "cmpctblock failed. send getdata block %s peer=%d\n", strBlockInfo(pindex), pfrom->id);
                    connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::GETDATA, vInv));
                    return true;
                }
        ...
        if (fBlockReconstructed) {
            ...
            if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS)) {
                ...
                MarkBlockAsReceived(pblock->GetHash(), pfrom->id);
            }
        }
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fReindex) // Ignore blocks received while reindexing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                LogPrint("block", "blocktxn %s FAILED. send getdata block peer=%d\n", strBlockInfo(pindex), pfrom->id);
    ...
    else if (strCommand == NetMsgType::HEADERS && !fReindex) // Ignore headers received while reindexing
    {
        ...
        if (nodestate->nUnconnectingHeaders > 0) {
            LogPrint("block", "resetting nUnconnectingHeaders (%d -> 0) peer=%d\n", nodestate->nUnconnectingHeaders, pfrom->id);
        }
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !chainActive.Contains(pindexWalk) && vToFetch.size() <= (nodestate->nMaxInFlight/10 + pfrom->nBlocksToBeProcessed)) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                BOOST_REVERSE_FOREACH(const CBlockIndex *pindex, vToFetch) {
                    if (nodestate->nBlocksInFlight - pfrom->nBlocksToBeProcessed >= (int)nodestate->nMaxInFlight/10) {
                    ...
                    if (nodestate->nBlocksInFlight - pfrom->nBlocksToBeProcessed < (int)nodestate->nMaxInFlight/10 && vToFetch.size() > 1)
    ...
    else if (strCommand == NetMsgType::BLOCK && !fReindex) // Ignore blocks received while reindexing
    {
        ...
        const uint256 hash(pblock->GetHash());
        {
            ...
            forceProcessing |= MarkBlockAsReceived(hash, pfrom->id);
            // mapBlockSource is only used for sending reject messages and DoS scores,
            // so the race between here and cs_main in ProcessNewBlock is fine.
            mapBlockSource.emplace(hash, std::make_pair(pfrom->GetId(), true));
        }
        bool fNewBlock = false;
        bool fSlowBiter = false;
        ProcessNewBlock(chainparams, pblock, forceProcessing, &fNewBlock, &fSlowBiter);
        if (fSlowBiter && GetBoolArg("-dynmax", false)) {
            // This node is slowing down chain Activation, reduce nBlocksInFlight to compensate
            int num = State(pfrom->id)->nMaxInFlight;
            if (num > 0 && vNodesSize == MAX_OUTBOUND_CONNECTIONS || num > 10) {
                LogPrint("block", "nMaxInFlight %d -> %d peer=%d\n", num*0.1, (num-1)*0.1, pfrom->id);
                State(pfrom->id)->nMaxInFlight--; num--;
                if (num/10 < 1)
                    pfrom->fDisconnect = 1;
            }
        }
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        if (!pfrom->fWhitelisted && pfrom->nServices && !(pfrom->nServices & NODE_XTHIN)) {
	    LogPrint("addrman", "recv getaddr. Ignoring. peer=%d\n", pfrom->id);
            return true;
	}
    ...
    else if (strCommand == NetMsgType::PONG)
    {
        ...
        if (!(sProblem.empty())) {
            LogPrint("net", "recv pong: %s, %x expected, %x received, %u bytes peer=%d\n",
                sProblem,
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrint("net", "recv feefilter %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), pfrom->id);
        }
    }
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_TX || inv.type == MSG_WITNESS_TX)
                LogPrint("tx", "recv notfound %s peer=%d\n", inv.ToString(), pfrom->id);
            else if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK) {
                LogPrint("block", "recv notfound %s peer=%d\n", inv.ToString(), pfrom->id);
                pfrom->fDisconnect = true;
                Misbehaving(pfrom->id, 100);
            } else
                LogPrint("net", "recv notfound %s peer=%d\n", inv.ToString(), pfrom->id);
        }
    }

    else {
        // Ignore unknown commands for extensibility
        LogPrint("net", "recv unknown command \"%s\" peer=%d\n", SanitizeString(strCommand), pfrom->id);
    }
...
bool SendMessages(CNode* pto, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    if (chainActive[pindex->nHeight] != pindex) {
                        LogPrint("block", "Announcing block %s %s not on main chain (tip=%s)\n",
                            hashToAnnounce.ToString(), strBlkInfo(pindex), chainActive.Tip()->GetBlockHash().ToString());
                    }
                    ...
                    if (!PeerHasHeader(&state, pindex)) {   // REBTODO - see what this function does
                        ...
                        LogPrint("block", "send inv%s block %s %s peer=%d\n", state.fSentGetblocks ? "" : "*", 
        ...
        //
        std::vector<CInv> vGetData;
        if ((fFetch || !IsInitialBlockDownload()) && state.nBlocksInFlight - pto->nBlocksToBeProcessed < (int)state.nMaxInFlight/10) {
            ...
            if (state.fExpectingBlocks && GetBoolArg("-dynmax", false)) {
                if (state.nBlocksInFlight == 0) {
                    LogPrint("block", "nMaxInFlight %d -> %d peer=%d\n", state.nMaxInFlight*0.1, state.nMaxInFlight*0.1+1, pto->id);
                    state.nMaxInFlight+=5;
                } /* else if (pto->nBlocksToBeProcessed == 0) {
                    LogPrint("block", "nMaxInFlight %d -> %d peer=%d\n", state.nMaxInFlight*0.1, (state.nMaxInFlight+1)*0.1, pto->id);
                    state.nMaxInFlight+=1;
                } */
                // Not expecting blocks for now as none are in flight
                state.fExpectingBlocks = false;
            }
            FindNextBlocksToDownload(pto->GetId(), state.nMaxInFlight/10 - state.nBlocksInFlight + pto->nBlocksToBeProcessed, vToDownload, staller, consensusParams);
            if (pto->nBlocksToBeProcessed + state.nMaxInFlight/10 <= 0) {
                pto->fDisconnect = 1;
                return true;
            }
            ...
            if (!fFetch && IsInitialBlockDownload() && state.nBlockPaused != 9) {
                ...
                if (state.nBlocksInFlight >= (int)state.nMaxInFlight)
                    strReason += strprintf("InFlight(%d)>=Max(%d) ", state.nBlocksInFlight, state.nMaxInFlight);
