    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        for (const UniValue& peer : batch[ID_PEERINFO]["result"].getValues()) {
            ...
            if (DetailsRequested()) {
                ...
                m_peers.push_back({addr, sub_version, conn_type, network, age, min_ping, ping, addr_processed, addr_rate_limited, last_blck, last_recv, last_send, last_trxn, peer_id, cpu_load, mapped_as, version, is_addr_relay_enabled, is_bip152_hb_from, is_bip152_hb_to, is_block_relay, is_outbound});
        ...
        if (DetailsRequested() && !m_peers.empty()) {
            ...
            result += strprintf("<->   type   net  mping   ping send recv  txn  blk  hb %*s%*s cpu%*s ",
            ...
            for (const Peer& peer : m_peers) {
                ...
                result += strprintf(
                    "%3s %6s %5s%7s%7s%5s%5s%5s%5s  %2s %*s%*s%*4s%*s%*i %*s %-*s%s\n",
