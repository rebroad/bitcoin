...
bool AddOrphanTx(const CTransaction& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    LogPrint("mempool2", "stored orphan tx %s (mapsz %u prevsz %u)\n", hash.ToString(),
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("%s: new best=%s (%d) log2_work=%.8g tx=%lu date=%s %f%% cache=%.1fMiB(%utx)\n", __func__,
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        //
        {
            ...
            if (fRevertToInv) {
                ...
                if (vHeaders.size() > 1) {
                    LogPrint("block", "send %u headers (%s, %s) peer=%d\n",
                            vHeaders.size(),
                ...
                if (IsThinBlocksEnabled() && IsChainNearlySyncd()) {
                    ...
                    if (HaveThinblockNodes() && CheckThinblockTimer(pindex->GetBlockHash())) {
                        ...
                        if (pto->mapThinBlocksInFlight.size() < 1 && pto->ThinBlockCapable()) { // We can only send one thinblock per peer at a time
                            ...
                            LogPrint("thin", "send get_xthin %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                        ...
                        if (pto->mapThinBlocksInFlight.size() < 1 && pto->ThinBlockCapable()) { // We can only send one thinblock per peer at a time
                            ...
                            vGetData.push_back(CInv(MSG_BLOCK, pindex->GetBlockHash()));
                            LogPrint("net", "send getdata block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                                     pindex->nHeight, pto->id);
                        }
                        MarkBlockAsInFlight(pto->GetId(), pindex->GetBlockHash(), consensusParams, pindex);
                    }
