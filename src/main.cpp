...
struct CNodeState {
    ...
    //! Whether we have recently requested headers.
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s:0x%08x (%d)  dw=%.8g tx=%lu date=%s %f%%\n", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Tip()->nVersion, chainActive.Height(),
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave) {
                    ...
                    if (theirheight >= chainActive.Height()-2)
                        LogPrint("block", "inv %s (%d) from peer=%d\n", inv.ToString(), theirheight, pfrom->id);
                }
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint("block", "sending getheaders (%d) %s to peer=%d\n", pindexBestHeader->nHeight, inv.hash.ToString(), pfrom->id);
                    ...
                    if (CanDirectFetch(chainparams.GetConsensus()) &&
                        ...
                        if (nodestate->fProvidesHeaderAndIDs && !(pfrom->GetLocalServices() & NODE_WITNESS)) {
                            LogPrint("block", "getdata cmpct %s to peer=%d\n", inv.ToString(), pfrom->id);
                            vToFetch.push_back(CInv(MSG_CMPCT_BLOCK, inv.hash));
                        } else {
                            LogPrint("block", "getdata %s to peer=%d\n", inv.ToString(), pfrom->id);
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            if (!nHeightStart) nHeightStart = pindex->nHeight;
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        assert(pindex);
        LogPrint("block", "from peer %d: cmpctblock %s (%d)\n", pfrom->id, cmpctblock.header.GetHash().ToString(), pindex->nHeight);
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (req.indexes.empty()) {
                    ...
                    LogPrint("block", "to peer %d: getblocktxn %s (%d) %d indexes\n", pfrom->id, req.blockhash.ToString(), pindex->nHeight, i);
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint("block", "from peer %d: blocktxn %s\n", pfrom->id, resp.blockhash.ToString());
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            LogPrint("block", "to peer %d: getdata %s\n", pfrom->id, invs[0].ToString());
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        BOOST_FOREACH(const CBlockHeader& header, headers) {
            ...
            LogPrint(nodestate->fExpectingHeaders ? "block2" : "block", "header %s (%d) from peer=%d\n", header.GetHash().ToString(), pindexLast->nHeight, pfrom->id);
        }
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        LogPrint("addrman", "from peer=%d getaddr. Pushing %d addresses. OldToSend=%d NewToSend=%d\n", pfrom->id, nCount, nOldToSend, pfrom->vAddrToSend.size());
    }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !pto->fFeeler && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                LogPrint("block2", "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->id, pto->nStartingHeight);
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    CBlockHeaderAndShortTxIDs cmpctblock(block);
                    LogPrint("block", "sending cmpctblock %s (%d) size=%d to peer %d\n",
                            vHeaders.front().GetHash().ToString(), nHeightEnd, sizeof(cmpctblock), pto->id);
                    pto->PushMessageWithFlag(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::CMPCTBLOCK, cmpctblock);
                    state.pindexBestHeaderSent = pBestIndex;
                } else if (state.fPreferHeaders) {
                    if (vHeaders.size() > 1) {
                        LogPrint("block", "sending %u headers (%d to %d) to peer=%d\n",
                                vHeaders.size(), nHeightStart, nHeightEnd, pto->id);
                    } else {
                        LogPrint("block", "sending header %s (%d) to peer=%d\n",
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrint("block", "sending inv block %s (%d) to peer=%d\n",
