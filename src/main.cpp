                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee)
{
    ...
    if (tx.IsCoinBase())
        return state.DoS(100, false, // coinbase as individual tx
                         REJECT_INVALID, "coinbase");

    // Rather not work on nonstandard transactions (unless -testnet/-regtest)
    string reason;
    if (Params().RequireStandard() && !IsStandardTx(tx, reason))
        return state.DoS(0, false, // nonstandard transaction
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (!MoneyRange(coins->vout[prevout.n].nValue) || !MoneyRange(nValueIn))
                return state.DoS(100, false, // txin values out of range
        ...
        nFees += nTxFee;
        if (!MoneyRange(nFees))
            return state.DoS(100, false, // nFees out of range
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (fEnforceBIP30) {
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            if (coins && !coins->IsPruned())
                return state.DoS(100, false, // tried to overwrite transaction
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOps > MAX_BLOCK_SIGOPS)
            return state.DoS(100, false, // too many sigops
                             REJECT_INVALID, "bad-blk-sigops");

        if (!tx.IsCoinBase())
        {
            if (!view.HaveInputs(tx))
                return state.DoS(100, false, // inputs missing/spent
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion < 3 && IsSuperMajority(3, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams))
        state.Invalid(error("%s : nVersion=2 block", __func__), REJECT_OBSOLETE, "bad-version");
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (send && !fAntisocial && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type != MSG_BLOCK) {
                if (!fAlreadyHave) {
                    ...
                    if (!fImporting && !fReindex && !fAntisocial)
                        pfrom->AskFor(inv);
                } else
    ...
    else if (strCommand == "getheaders")
    {
        ...
        LogPrint("net", "received getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString(), pfrom->id);
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        BOOST_FOREACH(const CBlockHeader& header, headers) {
            ...
            if (pindexLast) {
                if (!HeightStart) HeightStart = pindexLast->nHeight;
                HeightEnd = pindexLast->nHeight;
            }
        }
        ...
        LogPrint("net", "received %d headers (%d to %d) from peer=%d (startheight:%d)\n", nCount,
            HeightStart, HeightEnd, pfrom->id, pfrom->nStartingHeight);

        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            // Headers message had its maximum size; the peer may have more headers.
            // TODO: optimize: if pindexLast is an ancestor of chainActive.Tip or pindexBestHeader, continue
            // from there instead.
            LogPrint("net", "more getheaders (%d) to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom->id, pfrom->nStartingHeight);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (nNow - state.tLastDebugInfo > 5*60*1000*1000) {
            state.tLastDebugInfo = nNow;
            int nNewBlkInvsReceived = state.nNewBlkInvsReceived;
            state.nNewBlkInvsReceived = 0;
            int nOldBlkInvsReceived = state.nOldBlkInvsReceived;
            state.nOldBlkInvsReceived = 0;
            int nBlockInvsSent = state.nBlockInvsSent;
            state.nBlockInvsSent = 0;
            int nNewTxInvsReceived = state.nNewTxInvsReceived;
            state.nNewTxInvsReceived =0;
            int nOldTxInvsReceived = state.nOldTxInvsReceived;
            state.nOldTxInvsReceived = 0;
            int nTxInvsSent = state.nTxInvsSent;
            state.nTxInvsSent = 0;
            int nHeadersReceived = state.nHeadersReceived;
            state.nHeadersReceived = 0;
            int nBlksRequested = state.nBlksRequested;
            state.nBlksRequested = 0;
            int nBlksReceived = state.nBlksReceived;
            state.nBlksReceived = 0;
            int nTxsRequested = state.nTxsRequested;
            state.nTxsRequested = 0;
            int nTxsReceived = state.nTxsReceived;
