...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it->second->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
            LogPrint("block", "Peer %d sent us a getblocktxn for a block > %i deep", pfrom->id, MAX_BLOCKTXN_DEPTH);
            return true;
        }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            LogPrint("block", "more getheaders (%s%d) to end to peer=%d (startheight:%d)\n", strDesc, pindexContinue->nHeight, pfrom->id, pfrom->nStartingHeight);
