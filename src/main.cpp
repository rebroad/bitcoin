...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            {
                if (!fQuietInitial || CaughtUp())
                    printf("  getblocks stopping at %d %s (%u bytes)\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(10,15).c_str(), nBytes);
                break;
            }
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    loop
    {
              ...
              vRecv.size() >= nMessageSize) {
                ...
                if ((int)vRecv.size() == pfrom->nBlockBytes) {
                    ...
                    if (pfrom->nStuckDB < 3) {
                        ...
                        nBlockStuckNodes++;
                        fNodeSummary = true;
                        printf("given up %s (size %d) from %s", pfrom->WaitingForBlock.ToString().c_str(),
        ...
        if (!fRet)
            ...
            break; // Helps avoid dropping messages from peers when processing blocks.
    }
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // REBTODO - If we're short on providers, kick the stuck ones,
        // so we can reconnect and use them.
