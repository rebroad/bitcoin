        ...
        if (itOld != mapBlockIndex.end() && itOld->second->nChainWork > 0) {
            if (state->pindexBestKnownBlock == NULL || itOld->second->nChainWork >= state->pindexBestKnownBlock->nChainWork) {
                ...
                if (prevBestKnownBlock != state->pindexBestKnownBlock)
                    LogPrint("blockbestknown", "%s: BestKnownBlock: %d -> %d peer=%d\n", __func__, prevBestKnownBlock ? prevBestKnownBlock->nHeight : -1, state->pindexBestKnownBlock->nHeight, nodeid);
            }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s (%d) ver=0x%08x date=%s tx=%lu %f%%\n", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->nVersion,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
      (unsigned long)chainActive.Tip()->nTx,
      Checkpoints::GuessVerificationProgress(chainParams.Checkpoints(), chainActive.Tip())*100);
...
{
    LOCK(cs_main);
    fWitnessActive = (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == THRESHOLD_ACTIVE);
    return fWitnessActive;
}
...
void PeerLogicValidation::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    if (!fInitialDownload) {
        ...
        connman->ForEachNode([nNewHeight, &vHashes](CNode* pnode) {
            ...
            if (nNewHeight > (state->pindexBestKnownBlock ? state->pindexBestKnownBlock->nHeight : pnode->nStartingHeight - 2000)) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end()) {
                    ...
                    LogPrint("block", "recv getdata %d - UNKNOWN! peer=%d\n", inv.ToString(), pfrom->id);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        LogPrintf("recv version: %s: version %d, blocks=%d, relay=%s, services=%08x us=%s%s, peer=%d\n",
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs)) {
            ...
            LogPrint("mempool2", "AcceptToMemoryPool: peer=%d: accepted %s size=%u (poolsz %u txn, %u kB)\n",
                pfrom->id,
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (stateDummy.IsInvalid(nDos) && nDos > 0)
                        {
                            ...
                            LogPrintf("   invalid orphan tx %s (from peer=%d)\n", orphanHash.ToString(), fromPeer);
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fRequested && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (status == READ_STATUS_INVALID) {
                    LogPrintf("recv cmpctblock %s (%d) INVALID! peer=%d\n", pindex->GetBlockHash().ToString(), pindex->nHeight, pfrom->id);
                ...
                if (!fRequested && mapBlocksInFlight.size() == 1 && pindex->pprev->IsValid(BLOCK_VALID_CHAIN)) {
                ...
                if (req.indexes.empty()) {
                    ...
                    blockTxnMsg << txn;
                    LogPrint("block", "Calling ProcessMessage(BLOCKTXN) peer=%d\n", pfrom->id);
                    return ProcessMessage(pfrom, NetMsgType::BLOCKTXN, blockTxnMsg, nTimeReceived, chainparams, connman);
                } else {
                    req.blockhash = pindex->GetBlockHash();
                    int nSendSize = pfrom->PushMessage(NetMsgType::GETBLOCKTXN, req);
                    LogPrint("block", "send getblocktxn %s (%d) indexes=%d size=%d peer=%d\n", req.blockhash.ToString(), pindex->nHeight, i, nSendSize, pfrom->id);
                }
            }
        } else { // block is more than 2 blocks ahead of our current tip
            if (fRequested) {
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (fBlockRead) {
            ...
            if (state.IsInvalid(nDoS)) {
                ...
                if (nDoS > 0) {
                    LOCK(cs_main);
                    LogPrintf("block %s (%d) IsInvalid peer=%d\n", block.GetHash().ToString(), mi->second->nHeight, pfrom->id);
                    Misbehaving(pfrom->GetId(), nDoS);
                } else
                    LogPrint("block", "block %s (%d) IsInvalid peer=%d\n", block.GetHash().ToString(), mi->second->nHeight, pfrom->id);
            }
        }
    }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        BOOST_FOREACH(const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                LogPrintf("recv header %s prev %s does not equal %s (%d) peer=%d\n", header.GetHash().ToString(), header.hashPrevBlock.ToString(), pindexLast->GetBlockHash().ToString(), pindexLast->nHeight, pfrom->id);
                Misbehaving(pfrom->GetId(), 20);
                return true;
            }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool forceProcessing = pfrom->fWhitelisted && !IsInitialBlockDownload();
        {
            ...
            forceProcessing |= MarkBlockAsReceived(block.GetHash(), pfrom->id);
        }
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (nDoS > 0) {
                LOCK(cs_main);
                LogPrintf("block %s%s IsInvalid peer=%d\n", block.GetHash().ToString(), strHeight, pfrom->id);
                Misbehaving(pfrom->GetId(), nDoS);
            } else
                LogPrint("block", "block %s%s IsInvalid peer=%d\n", block.GetHash().ToString(), strHeight, pfrom->id);
        }
...
bool ProcessMessages(CNode* pfrom, CConnman& connman)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
	    ...
	    if (!msg.complete() && (strCommand == NetMsgType::BLOCK || strCommand == NetMsgType::CMPCTBLOCK || strCommand == NetMsgType::BLOCKTXN))
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    int nSize = pto->PushMessageWithFlag(state.fWantsCmpctWitness ? 0 : SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::CMPCTBLOCK, cmpctblock);
