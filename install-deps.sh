...
install_packages() {
    ...
    if [ ${#to_install[@]} -gt 0 ]; then
        ...
        return 1  # No packages were installed
    fi
}
...
run_configure() {
    ...
    print_status "Running ./configure $configure_options to check for missing dependencies..."

    # Capture both stdout and stderr
    local output
    if output=$(./configure $configure_options 2>&1); then
        ...
        # Check if wallet was enabled
        if grep -q "enable_wallet.*=.*yes" config.log 2>/dev/null || grep -q "ENABLE_WALLET.*1" config.log 2>/dev/null; then
            print_success "Wallet support enabled!"
        fi
...
install_dependencies() {
    ...
    # Install SQLite development package (needed for pkg-config)
    print_status "Installing SQLite development package..."
    ...
    # If configure failed, check for Qt development packages
    print_status "Checking for Qt development packages..."
    local qt_dev_packages=("qttools5-dev" "qttools5-dev-tools" "qtbase5-dev" "qtbase5-dev-tools" "libqt5gui5" "libqt5core5a" "libqt5dbus5" "qtwayland5")
    local qt_installed=false
    
    if install_packages "${qt_dev_packages[@]}"; then
        qt_installed=true
    fi
    ...
    if [ "$qt_installed" = true ] || [ "$fallback_installed" = true ]; then
        ...
        print_warning "No new packages were installed, but configure is still failing"
        print_status "This might be due to version incompatibilities or missing system libraries"
        print_status "Consider using the depends system: make -C depends"
        return 1
    fi
